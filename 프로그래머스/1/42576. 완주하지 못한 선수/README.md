...
### 접근
동명이인이 있어 set으로는 처리 불가. 딕셔너리로 참가자 수를 센 뒤
완주자를 하나씩 차감해 1이 남은 이름을 반환.

### 시간 복잡도
O(N). N ≤ 100,000이므로 충분.

### 막힌 지점
- 리스트끼리 `-` 연산 불가 → 파이썬 리스트는 뺄셈 미지원
- `count[name] - 1`로 계산만 하고 대입을 누락 → `-=` 또는 `=` 필요
- 인자로 받은 `participant`를 빈 리스트로 덮어씀

### 다른 풀이
`collections.Counter`끼리 `-` 연산 시 0 이하 항목이 자동 제거됨.
