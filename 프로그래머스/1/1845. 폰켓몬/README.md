...
### 접근
가져갈 수 있는 마리 수(`len(nums) // 2`)와 전체 종류 수(`len(set(nums))`) 중
작은 값이 답. set이 중복을 제거해주므로 종류 수는 한 번에 구할 수 있다.

### 시간 복잡도
O(N). set 변환이 O(N).

### 막힌 지점
- 문제를 잘못 읽음 "선택 방법의 가짓수"와 "한 번에 손에 쥔 종류 수"를 혼동.
  가져가는 마리 수를 넘는 종류를 가질 수 없다는 점을 놓쳤다.

### 다른 풀이
없음 — 다른 사람 풀이도 접근은 동일. 한 줄로 합쳤을 뿐이며
`/`를 써서 float를 반환하는 점은 오히려 부정확하다.
